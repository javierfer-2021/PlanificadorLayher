...
export class FrmCompraImportarComponent implements OnInit {
  ...
  dgConfigLineas: DataGridConfig = new DataGridConfig(this.arrayLineasEntrada, this.cols, 300, ConfiGlobal.lbl_NoHayDatos);
  ...
  async obtenerDatosCompraERP() : Promise<void> {
    /*
    if(this.WSDatos_Validando) return;
    ...
    (await this.planificadorService.cargarEntrada_from_ERP(this.str_txtContrato)).subscribe(
      datos => {
        if(Utilidades.DatosWSCorrectos(datos)) {
          ...
          this.dgConfigLineas.dataSource = this.arrayLineasEntrada;
    ...
    */
    ...
    try {
      const observable = await this.planificadorService
        .cargarEntrada_from_ERP(this.str_txtContrato);
  ...
  async btnBuscarDocumento() {
    if (Utilidades.isEmpty(this.str_txtContrato)) {
      ...
      await this.obtenerDatosCompraERP();
    };      
  }
  ...
  onFechaPrevistaValueChanged(e){
    if ((this.contratoValido) && (!Utilidades.isEmpty(this._entrada.FechaPrevista)) && (this._entrada.FechaPrevista.getFullYear()<1900)) {
      this._entrada.FechaPrevista = Utilidades.year2to4digits(this._entrada.FechaPrevista);
    }
  }
 
  onFechaConfirmacionValueChanged(e){
    if ((this.contratoValido) && (!Utilidades.isEmpty(this._entrada.FechaConfirmada)) && (this._entrada.FechaConfirmada.getFullYear()<1900)) {
  ...
  cerrarEliminarMultiplesLineas(lineas){
    if (lineas!=null) {
      ...
      this.arrayLineasEntrada = lineas;
      this.dgConfigLineas = new DataGridConfig(this.arrayLineasEntrada, this.cols, this.dgConfigLineas.alturaMaxima, ConfiGlobal.lbl_NoHayDatos);
      this.dgConfigLineas.actualizarConfig(true,false,'standard',true,false);     
      // config extra
      this.ConfigGrid = true;
