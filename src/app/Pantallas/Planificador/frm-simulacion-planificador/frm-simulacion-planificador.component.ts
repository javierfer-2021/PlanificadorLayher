...
@Component({
  selector: 'app-frm-simulacion-planificador',
  templateUrl: './frm-simulacion-planificador.component.html',
  styleUrls: ['./frm-simulacion-planificador.component.css']
})
export class FrmSimulacionPlanificadorComponent implements OnInit {
...
btnAciones: BotonPantalla[] = [
  { icono: '', texto: this.traducir('frm-planificacion.btnSalir', 'Salir'), posicion: 1, accion: () => {this.btnSalirSimulacion()}, tipo: TipoBoton.danger, activo: true, visible: true },
...
async getPlanificacion(){
  ...
  (await this.planificadorService.getDatosSimulacionPlanificador(this._salida.IdSalida)).subscribe(
    datos => {
      ...
      this.WSDatos_Validando = false;
      this.ocultarPanelProceso();
      Utilidades.compError(error, this.router, 'frm-simulacion-planificador');  
      //console.log(error);
    }
  );
}
...
async insertarArticulo(idArticulo:string,unidades:number,observaciones:string){
  ...
  (await this.planificadorService.insertarArticuloPlanificador(this.oOfertaSeleccionada.IdSalida,idArticulo,unidades,observaciones)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //console.log(datos);
        Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSInsertarArticulos','Artículo Insertado correctamente'));           
        this.WSDatos_Validando = false;
        this.limpiarControles(true);
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSInsertarArticulos','Error WS insertando artículo')); 
      }
    }, error => {
      this.WSDatos_Validando = false;
      Utilidades.compError(error, this.router, 'frm-simulacion-planificador');        
...
async eliminarArticulo(idArticulo:string,motivo:string){
  ...
  (await this.planificadorService.eliminarArticuloPlanificador(this.oOfertaSeleccionada.IdSalida,idArticulo,motivo)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //console.log(datos);
        Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSEliminarArticulos','Artículo Eliminado correctamente'));           
        this.WSDatos_Validando = false;
        this.limpiarControles(true);
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSEliminarArticulos','Error WS eliminando artículo')); 
      }
    }, error => {        
      this.WSDatos_Validando = false;
      Utilidades.compError(error, this.router, 'frm-simulacion-planificador');        
...
async cambiarArticulo(idArticulo:string,motivoEliminar:string,idNewArticulo:string,unidades:number,observaciones:string){
  ...
  (await this.planificadorService.cambiarArticuloPlanificador(this.oOfertaSeleccionada.IdSalida,
    ...
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //console.log(datos);
        Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSCambiarArticulos','Artículo Cambiado correctamente'));           
        this.WSDatos_Validando = false;
        this.limpiarControles(true);
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSCambiarArticulos','Error WS cambiando artículo')); 
      }
    }, error => {        
      this.WSDatos_Validando = false;
      Utilidades.compError(error, this.router, 'frm-simulacion-planificador');        
...
async actulizarArticuloValorSecundario(idArticulo:string,valor:boolean){
  ...
  (await this.planificadorService.actualizarArticuloValorSecundario(idArticulo,this.oOfertaSeleccionada.IdAlmacen,valor)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSActualizarValorSecunadrio','Artículo Actualizado correctamente')); 
        let index:number = this.arrayArts.findIndex(e => e.IdArticulo==idArticulo);
        if (index>=0) { this.arrayArts[index].Prioridad = valor; }
        this.WSDatos_Validando = false;
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSActualizarValorSecunadrio','Error WS actualizando valor prioridad artículo')); 
      }
    }, error => {        
      this.WSDatos_Validando = false;
      Utilidades.compError(error, this.router, 'frm-simulacion-planificador');        
...
async planificarContratoSalida(idSalida:number){
  ...
  (await this.planificadorService.planificarContratoSalida(idSalida)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSPlanificarContrato','Contrato Planificado'));           
        this.WSDatos_Validando = false;
        this.limpiarControles(true);
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSPlanificarContrato','Error WS planificar contrato salida')); 
      }
    }, error => {
      //console.log(error);        
      this.WSDatos_Validando = false;
...
async desPlanificarContratoSalida(idSalida:number){
  ...
  (await this.planificadorService.desPlanificarContratoSalida(idSalida)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSDesPlanificarContrato','Contrato DES-Planificado'));           
        this.WSDatos_Validando = false;
        this.limpiarControles(true);
      } else {          
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSDesPlanificarContrato','Error WS DES-planificar contrato salida')); 
      }
    }, error => {
      //console.log(error);        
      this.WSDatos_Validando = false;
...
async actializarPlanificacionLineas(datosLinea:modLineaPlanificador, listaSalidas:Array<modLineaPlanificador>){
  ...
  (await this.planificadorService.actualizarLineasPlanificadas(datosLinea.IdSalida,datosLinea.IdArticulo,datosLinea.UndServidas,listaSalidas)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSActualizarPlanificacionLineas','Articulos RE-Planificado'),'success',1000);           
        ...
        this.WSDatos_Validando = false;
        Utilidades.MostrarErrorStr(this.traducir('frm-simulacion-planificador.msgError_WSActualizarPlanificacionLineas','Error WS RE-Planificar artículo')); 
      }
    }, error => {
      //console.log(error);        
      this.WSDatos_Validando = false;
...
async borrarDatosSimulacionSalida(){
  if(this.WSDatos_Validando) return;    

  this.WSDatos_Validando = true;
  (await this.planificadorService.BorrarSimulacionSalidaCSV(this._salida.IdSalida)).subscribe(
    datos => {
      if(Utilidades.DatosWSCorrectos(datos)) {
        //Utilidades.MostrarExitoStr(this.traducir('frm-simulacion-planificador.msgOk_WSActualizarPlanificacionLineas','Articulos RE-Planificado'),'success',1000);           
        this.WSDatos_Validando = false;
...
async eliminarArticuloSalida(articulo:SalidaLinea){
  let continuar = <boolean>await Utilidades.ShowDialogString(this.traducir('frm-simulacion-planificador.MsgEliminarArticulo', 'El artículo '+articulo.IdArticulo+'-'+articulo.NombreArticulo+' sera eliminado de la planificación.'+'<br>¿Esta seguro que desea Continuar?'), this.traducir('frm-simulacion-planificador.TituloConfirmar', 'Confirmar'));  
  if (!continuar) return;
  else {
...
cerrarSeleccionarArticulo(e){
  if (e != null) {
    ...
    if (index<0) {
      ...
      Utilidades.ShowDialogAviso(this.traducir('frm-simulacion-planificador.msgError_ArticuloYaExistente','No insertado: Artículo indicado ya incluido.<br>Modifique unidades manualmente'))
    }
  }
...
async btnPlanificarContrato(){
  let msgConfirmacion = 'Contrato: '+ this.oOfertaSeleccionada.Contrato +'<br>'
                      ...
                      + this.traducir('frm-simulacion-planificador.MsgPlanificarContrato','¿Esta seguro que desea continuar?');
  let continuar = <boolean>await Utilidades.ShowDialogString(msgConfirmacion, this.traducir('frm-simulacion-planificador.TituloPlanificarContrato', 'Planificar Contrato'));  
  if (!continuar) return
  else {    
...
async btnDESplanificarContrato(){
  let msgConfirmacion = 'Contrato: '+ this.oOfertaSeleccionada.Contrato +'<br>'
                      + 'Cliente: ' + this.oOfertaSeleccionada.IdClienteERP + ' - ' + this.oOfertaSeleccionada.NombreCliente +'<br><br>'
                      + this.traducir('frm-simulacion-planificador.MsgPlanificarContrato','¿Esta seguro que desea continuar?');
  let continuar = <boolean>await Utilidades.ShowDialogString(msgConfirmacion, this.traducir('frm-simulacion-planificador.TituloDesplanificarContrato', 'DES-Planificar Contrato'));  
  if (!continuar) return
  else {    
    //alert('Planificar CONTRATO: '+this.oOfertaSeleccionada.Contrato);
...
async cambiarContratoSeleccionado(index:number){
  let msgConfirmacion = 'Contrato: '+ this.arrayCabeceras[index].Contrato +'<br>'
                      ...
                      + this.traducir('frm-simulacion-planificador.MsgCambiarContratoSeleccionado','¿Esta seguro que desea continuar?');
  let continuar = <boolean>await Utilidades.ShowDialogString(msgConfirmacion, this.traducir('frm-simulacion-planificador.TituloCambiarContrato', 'Cambiar contrato seleccionado'));  
  if (!continuar) return
  else {
...
async DESplanificarContrato(index:number){
  let msgConfirmacion = 'Contrato: '+ this.arrayCabeceras[index].Contrato +'<br>'
                      + 'Cliente: ' + this.arrayCabeceras[index].IdClienteERP + ' - ' + this.arrayCabeceras[index].NombreCliente +'<br><br>'
                      + this.traducir('frm-simulacion-planificador.MsgDesplanificarContrato','¿Esta seguro que desea continuar?');
  let continuar = <boolean>await Utilidades.ShowDialogString(msgConfirmacion, this.traducir('frm-simulacion-planificador.TituloDesplanificarContrato', 'DES-Planificar Contrato'));  
  if (!continuar) return
  else {    
...
async cancelarContrato(index:number){
  let msgConfirmacion = 'Contrato: '+ this.arrayCabeceras[index].Contrato +'<br>'
                      + 'Cliente: ' + this.arrayCabeceras[index].IdClienteERP + ' - ' + this.arrayCabeceras[index].NombreCliente +'<br><br>'
                      + this.traducir('frm-simulacion-planificador.MsgCancelarContrato','¿Esta seguro que desea continuar?');
  let continuar = <boolean>await Utilidades.ShowDialogString(msgConfirmacion, this.traducir('frm-simulacion-planificador.TituloCancelarContrato', 'Cancelar Contrato'));  
  if (!continuar) return
  else {    
...
public btnSalirSimulacion(){
  this.borrarDatosSimulacionSalida();
  this.location.back();
}
